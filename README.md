...
Current status: Explorer-hosted UI scaffold. The mod targets `explorer.exe`, adds a top-right `Expandable folders` checkbox for each visible File Explorer window, and covers the native file-list area with an empty view when the checkbox is enabled.
...
- A top-right Explorer-owned checkbox named `Expandable folders`.
